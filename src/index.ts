...
const getZkFetchProof = async (idProtocol: string) => {
    try {
        const proof =  await reclaimClient.zkFetch(
          `https://eze-api.vercel.app/staking/${idProtocol}`,
          {
            method: "GET",
          },
          {
    ...
    const message = `Hello, this is a signed message from the Eze Finance Service Manager.`;
    ...
    const tx = await ezeFinanceServiceManager.respondToApproveTask(
...
const registerOperator = async () => {
    ...
    const operatorDigestHash = await avsDirectory.calculateOperatorAVSRegistrationDigestHash(
        wallet.address, 
        await ezeFinanceServiceManager.getAddress(), 
        salt, 
        expiry
    );
...
const monitorNewTasks = async () => {
    ...
    ezeFinanceServiceManager.on("NewEzeTaskCreated", async (taskIndex: number, task: any) => {
        try {
